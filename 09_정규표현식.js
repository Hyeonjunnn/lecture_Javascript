window.onload = () => {
    ...
    btn3.addEventListener('click', () => {
        ...
        // 반복 검색
        // {m,n}로 앞선 패턴이 최소 m번 최대 n번 반복되는 문자열을 의미한다.
        div3.innerHTML = `/a{1,2}/g: ${str.replace(/a{1,2}/g, '($&)')}<br>`;
        // {3} = {3,3} 앞선 패턴이 최소 3번 최대 3번 반복되는 문자열을 의미한다.
        ...
        // ?는 앞선 패턴이 최대 한 번 반복되는 문자열을 의미한다. {0,1}
        div3.innerHTML += `/a?/g: ${str.replace(/a?/g, '($&)')}<br>`;
        ...
        div3.innerHTML += `/[aj^]/ig: ${str.replace(/[aj^]/ig, '($&)')}<br>`;
        // [...] 내의 -는 범위 지정을 의미한다.
        div3.innerHTML += `/[a-z]/ig: ${str.replace(/[a-z]/g, '($&)')}<br>`;
        div3.innerHTML += `/[A-Z]/ig: ${str.replace(/[A-Z]/g, '($&)')}<br>`;
        div3.innerHTML += `/[0-9]/ig: ${str.replace(/[0-9]/g, '($&)')}<br>`;
        div3.innerHTML += `/[A-Z0-9]/ig: ${str.replace(/[A-Z0-9]/g, '($&)')}<br>`;
