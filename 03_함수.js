...
// btn2.addEventListener('click', () => {
    ...
    // 호출될 때 형태가 특정 객체의 메소드 형태가 아니므로 this에 선택 객체가 바인딩된다.
    // inner(); // window 반환

    // bind 메소드를 통해서 바깥쪽 함수의 this를 inner 함수의 this로 직접 연결할 수 있다.
    // inner.bind(this)(); // btn2 반환
...
function min (...numbers) {
    let sortNums = numbers.sort((left, right) => left - right);
    let minNum = sortNums.at(0);
...
btn3.addEventListener('click', () => {
    ...
    let encodeUrl = 'http://www.example.com?name=%ED%99%8D%EA%B8%B8%EB%8F%99&age=24&address=%EA%B2%BD%EA%B8%B0%EB%8F%84%20%EA%B3%A0%EC%96%91%EC%8B%9C';;

    // encodeURI()
    //  - encodeURI() 함수는 URI를 인코딩하는데 사용된다.
    //  - , / ? = & ; : @ + $ # 은 인코딩하지 않는다.

    // decodeURI()
    //  - 인코딩된 URI를 디코딩하는데 사용된다.

    p6.innerHTML += `<h4>encodeURI(), decodeURI()</h4>`;
    p6.innerHTML += `encodeURI(query): ${encodeURI(url)}<br>`;
    p6.innerHTML += `decodeURI(query): ${decodeURI(encodeUrl)}<br>`;
