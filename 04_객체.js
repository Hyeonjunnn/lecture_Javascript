window.onload = () => {
    ...
    btn1.addEventListener('click', () => {
        ...
        div1.innerHTML += `product.pName: ${product.pName}<br>`;
        div1.innerHTML += `product.price: ${product.price}<br>`;
        // div1.innerHTML += `product.0: ${product.0}<br>`; // 숫자 프로퍼티는 마침표로 접근이 불가
        div1.innerHTML += `product.brand: ${product.brand}<br>`;
        div1.innerHTML += `product.spec: [${product.spec}]<br>`;
    ...
    btn4.addEventListener('click', () => {
        ...
        // 모든 학생의 정보다 담긴 배열을 출력 (이름, 영어 점수, 수학 점수, 총합, 평균)
        students.forEach(student => {
            document.getElementById('div4').innerHTML +=
                `이름: ${student.name},
                영어 점수: ${student.eng}, 
    ...
    function Student(name, eng, math) {
        ...
        // 프로토타입(prototype)
        // 프로토타입 객체는 어떤 객체의 상위 객체의 역할을 하는 객체로 공유 프로퍼티와 메소드
    ...
    btn7.addEventListener('click', () => {
        ...
        // Date 객체의 메소드 호출
        div7.innerHTML += `now.getYear(): ${now.getFullYear()}<br>`;
